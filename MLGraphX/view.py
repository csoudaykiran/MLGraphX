...
def upload(request):
    ...
    if request.method == 'POST':
        ...
        #data = pd.read_csv(r'D:\Downloads\ML-AUTOMATION-DJANGO-main\ML-AUTOMATION-DJANGO-main\DeployModel' + fs.url(uploaded_file.name))
        
        # Get the absolute file path
        file_path = os.path.abspath(fs.path(uploaded_file.name))
    ...
    return render(request, ['upload.html', 'home.html','knn.html'],column)
...
def result1(request):
    ...
    if 'submit' in request.GET:
        ...
        from sklearn.model_selection import train_test_split
        from sklearn.neighbors import KNeighborsClassifier
        x = data.iloc[:,feature].apply(pd.to_numeric, errors='coerce').values
        ...
        return render(request, "knn.html", {'y_pred': y_pred, 'acc': acc,'acc1':acc1, 'plot_div': plot_div,'column':column})
